...
> 后台管理入口不再使用全局密码，而是基于用户**角色**（`role`）：管理员登录后导航栏
> 右侧会出现「🛠 后台管理」按钮；普通用户看不到。设置管理员见下方「后台管理」一节。
...
- **入口**：管理员用户登录后，导航栏右侧出现「🛠 后台管理」按钮（普通用户无此入口）
- **设置管理员**：在 Supabase SQL Editor 执行（把 `你的用户名` 换成实际用户名）：
  ```sql
  update public.profiles set role = 'admin' where username = '你的用户名';
  ```
  可设多个管理员；如需取消管理员身份，把 `role` 改回 `user` 即可
- 功能：
  - 用户总览：注册用户数、总记录、总饮水量、近 7 天日活
  - 用户列表：分页展示（每页 20 人），支持**按名称搜索**、**按创建时间 / 最后活跃 / 名称排序**
  - ✏️ **修改用户名**：管理员可帮任何用户改名（改自己会同步当前会话）
  - 🔑 **重置密码**：用户忘记密码时，管理员可为其设置新密码
  - 🗑 **删除用户**：永久删除该用户及其全部记录/帖子/勋章
  - 📤 **导出全部数据**：一键备份所有用户数据（JSON，含 id 与角色）
  - 📥 **导入全部数据**：从备份文件恢复（会覆盖当前数据，需二次确认）

> 角色由数据库 `profiles.role` 控制，且已撤销匿名/普通用户对 `id`、`role` 两列的写权限（防提权）。
> 该设计适用于亲友/团队内部使用；如需金融级安全请改用 Supabase Auth 接入邮箱/手机号登录。
