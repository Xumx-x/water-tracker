...
- 数据库：Supabase PostgreSQL，八张表（**全部以 user_id 关联，无 username 外键**）
  - profiles 注意：id 上可能有**两个唯一约束并存**（主键 profiles_pkey + 冗余 profiles_id_key，v11.6 迁移遗留，子表外键绑定后者，勿删）
...
- **⚙️ 体验优化（v11.7）**：initApp 有会话时先隐藏登录层再异步加载（防刷新闪现）+ renderRoute 停留当前页；排行榜 `.rank-loading` 遮罩（setRankLoading）不清空旧数据；下拉菜单新增「👤 个人中心」（#page-profile：changeUsername 改用户名同步 SESSION_KEY/userNameMap + 改密码迁入，settings 密码卡移除）；后台 adminState 前端分页（20/页）+ adminSearchInput 搜索 + adminSortSelect 排序（最后活跃/创建时间/名称）+ renameUserOverlay 改名（改自己同步会话）
- **⚠️ 已知坑**：Supabase anon 默认禁用数据库聚合函数（PGRST123），排行榜/论坛计数必须前端拉明细再 JS 分组，或建 RPC/视图；jsdom 测试勿用 resources:'usable'（会真实加载 CDN supabase 覆盖 mock），用 beforeParse 注入；SQL 美元引用外层用 $do$ 内层用 $$（同标签不能嵌套）；mock 链式查询需含 gte/lte；mock 需模拟 profiles.username 唯一约束（23505）才能测改名冲突
