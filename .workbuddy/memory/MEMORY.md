...
- 数据库：Supabase PostgreSQL，八张表
  - `profiles`：username(PK) / password_hash(PBKDF2 格式 `pbkdf2$100000$salt$hex`) / goal / **custom_cups**(JSON 文本，快速记录自定义) / first_login / last_active
  - `water_records`：id(自增 PK) / username(FK 级联删) / date / time / amount
  - `forum_posts`：id / author(FK 级联删) / title(≤60) / content(≤2000) / created_at
  - `forum_likes`：id / post_id(FK 级联删) / username(FK) / **unique(post_id,username)** 防重复赞
  - `forum_comments`：id / post_id(FK 级联删) / username(FK) / content(≤500) / created_at
  - `honor_boards`：id / board_type(daily|weekly|monthly) / period / rank(1-3) / username(FK) / amount，**unique(board_type,period,username)** 幂等
  - `honor_badges`：PK(username,badge_type) / count（badge_type: daily|weekly|monthly|achieve）
  - `honor_achieve`：username(FK) / record_date / total / goal，**unique(username,record_date)** 防重复计分
...
- **🏆 荣誉墙（v11.5）**：pg_cron 定时（**北京时间=UTC+8**，日榜/达标 `0 17 * * *`/`30 17 * * *`、周榜 `0 17 * * 1`、月榜 `0 17 1 * *`）→ 生成函数 `generate_honor_board(+daily/weekly/monthly)` 与 `generate_honor_achieve()`；**冻结机制**=INSERT ON CONFLICT DO NOTHING RETURNING，仅新增成功才发勋章；函数日期按 `Asia/Shanghai` 计算；函数已 revoke PUBLIC 执行权限；honor 三表 RLS 只读；前端页面展示个人 4 勋章 + TOP50 总榜（明细拉取前端聚合）
- **⚠️ 已知坑**：Supabase anon 默认禁用数据库聚合函数（PGRST123），排行榜/论坛计数必须前端拉明细再 JS 分组，或建 RPC/视图；jsdom 测试勿用 resources:'usable'（会真实加载 CDN supabase 覆盖 mock），用 beforeParse 注入
